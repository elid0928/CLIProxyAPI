...
// Delete removes an auth entry from runtime state and deletes its persisted record.
func (m *Manager) Delete(ctx context.Context, id string) (*Auth, error) {
	...
	m.Remove(ctx, id)
	if errDelete := m.deletePersisted(ctx, removed); errDelete != nil {
		return removed.Clone(), errDelete
	}
